...
void Class_Tricycle_Chassis::Init(float __Velocity_X_Max, float __Velocity_Y_Max, float __Omega_Max, float __Steer_Power_Ratio)
{
    ...
    #endif
    //关节电机初始化
    Motor_Joint.Init(&hfdcan2, LK_Motor_ID_0x141, LK_Motor_Control_Method_IMU_ANGLE);
    Motor_Joint.PID_Angle.Init(10.f, 0.0f, 0.0f, 0.0f, Motor_Joint.Get_Output_Max(), Motor_Joint.Get_Output_Max());
    //底盘控制方式初始化
   ...
   switch (Chassis_Control_Type)
    {
        ...
        {
            Motor_Joint.Set_LK_Motor_Control_ID(LK_Motor_Control_Torque);
            Motor_Joint.Set_LK_Control_Status(LK_Motor_Control_Status_ENABLE);
            Motor_Joint.Set_LK_Motor_Control_Method(LK_Motor_Control_Method_ANGLE);
            //Motor_Joint.Set_Target_Angle((Count_test_joint / 1000) % 2 == 0 ? 0.0f : 1800);
            Motor_Joint.Set_Target_Current(0);
            Motor_Joint.Set_Target_Torque(0);
            Motor_Joint.TIM_PID_PeriodElapsedCallback();
            break;
        }
...
void Class_Tricycle_Chassis::Send_Data_To_VOFA(void)
{
    ...
    if (current_time - last_send_time > 20)
		{
			// float target_speed =  gm6020_control.speed_target;
			// float current_speed = gm6020_motor.speed_rpm;
			 last_send_time = current_time;
            float actual_angle = Motor_Joint.Get_Now_Angle() - 360.0f;
			 printf("%.2f\r\n",actual_angle);
		}
