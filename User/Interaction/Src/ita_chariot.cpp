...
{
    //底盘坐标系速度目标值 float
    float chassis_velocity_x = 0, chassis_velocity_y = 0, gimbal_pitch = 0; 
    //映射之后的目标速度 int16_t
    int16_t tmp_chassis_velocity_x = 0, tmp_chassis_velocity_y = 0, tmp_chassis_omega = 0, tmp_gimbal_yaw = 0;
    ...
    control_type = (uint8_t)(Referee_UI_Refresh_Status << 6 |  MiniPC_Status<< 5 | Fric_Status << 4 | Yaw_Encoder_Control_Status << 3 | Chassis_Logics_Direction << 2 | chassis_control_type);
    //设定速度
...
void Class_Chariot::Control_Chassis()
{
    ...
    switch(DR16_Left_Switch_Status){
        ...
        case(DR16_Switch_Status_DOWN): // 左下 上位机模式 底盘暂时失能
        {
...
{
    // 角度目标值
    float tmp_gimbal_yaw, tmp_gimbal_yaw_encoder, tmp_gimbal_yaw_imu, tmp_gimbal_pitch;
    // 遥控器摇杆值
...
void Class_Chariot::TIM1msMod50_Alive_PeriodElapsedCallback()
{
    ...
    if (mod50 == 50)
    {
        ...
        //TIM_Unline_Protect_PeriodElapsedCallback();
            ...
            Chassis.Motor_Joint.TIM_Alive_PeriodElapsedCallback();          
            if(mod50_mod3%3 == 0)
            {
            ...
            */ 
            ...
            Gimbal.Motor_Yaw_test.TIM_Alive_PeriodElapsedCallback();
